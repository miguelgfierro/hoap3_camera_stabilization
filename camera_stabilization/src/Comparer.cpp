...
int Comparador::calcular_errores()
{
    ...
    if( objectDescriptors->total > 2 && imageDescriptors->total > 2 )
    {
        ...
        for( int i = 0; i < 4; i++ )
        {
          std::cout << "src_corners= " << src_corners[i].x << "," << src_corners[i].y << std::endl;
          std::cout << "dst_corners= " << dst_corners[i].x << "," << dst_corners[i].y << std::endl;
        }
        //****Añadido por Miguel
        ...
        {	// Si se localiza un objeto plano se calcula el error de posicion y orientación a
            ...
            for( int i = 0; i < 4; i++ )
            {
                ...
                //****Añadido por Miguel: NO ENTIENDO COMO SE SACAN LOS ERRORES
                CvPoint r3 = dst_corners[i]; // r1=r3  Por que pones i%4?
